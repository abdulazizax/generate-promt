...
func New(openaiClient *openai.Client, logger logger.Logger) *gin.Engine {
	...
	docService, err := docs.NewService(context.Background(), option.WithCredentialsFile("service_account.json"))
	if err != nil {
		log.Fatalf("Unable to create Sheets service: %v", err)
	}
	handler := handlers.New(openaiClient, logger, sheetService, docService)
